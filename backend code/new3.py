...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "/home/pi/Downloads/MyFirstProject.json"
...
def text_detection():
    ...
    ##  rows,cols = img.shape
    M = cv2.getRotationMatrix2D((250/2,250/2),90,1)
    img = cv2.warpAffine(img,M,(250,250))
    vis      = img.copy()
    ...
    for channel in channels:
        erc1 = cv2.text.loadClassifierNM1('/home/pi/Documents/project1/trained_classifierNM1.xml')
        er1 = cv2.text.createERFilterNM1(erc1,10,0.00005,0.13,0.5,True,0.1)

        erc2 = cv2.text.loadClassifierNM2('/home/pi/Documents/project1/trained_classifierNM2.xml')
        er2 = cv2.text.createERFilterNM2(erc2,0.7)
...
##    cv2.imshow("output1", vis)
    ...
    if(np.shape(rects)[0]>=1):
        vis = cv2.warpAffine(vis,M,(250,250))
        vis = cv2.warpAffine(vis,M,(250,250))
        vis = cv2.warpAffine(vis,M,(250,250))
        cv2.imshow("Text detection result", vis)
##        global image_path,gray_original,vis
